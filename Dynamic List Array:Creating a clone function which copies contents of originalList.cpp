...
//Dynamic
List Array:Creating a clone function which copies contents of originalList
...
//
operations to be implemented: read, Modify, delete, isEmpty, insert,
countElements
...
       int maxSize; // useful to decide
if resizing (doubling the array size) is needed
...
       // Add
the code for the member function clone( ) that creates a copy

       // of
the List object on which it is called and returns the List
...
       List integerList(1); // we
will set the maxSize to 1 and double it as and when needed
...
       // call
the clone( ) member function on the integerList object and

       // store
the returned List object by the name cloneList

       // Then
call the print() function on cloneList and print its contents
